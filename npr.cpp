...
public:
    ...
    bool Winner()
    {
        if (score2 == 10)
        {
            DrawText("Ti si pobedio!!! ououououuu", GetScreenWidth() / 2 + 50, GetScreenHeight() / 2, 60, PINK);
            speed_x = 0;
            speed_y = 0;
            return true;
        }
        ...
        {
            DrawText("Ti si pobedio!!! ououououuu", 50, GetScreenHeight() / 2, 60, PINK);
            speed_x = 0;
            speed_y = 0;
            return true;
        }
    ...
    void Update(Paddle p1, Paddle p2, Sound reb, Sound faill, Sound pad)
    {
        ...
        bool overlapRight = (x + radius >= p1.x && x + radius <= p1.x + p1.width) ? true : false;
        bool overlapLeft = (x + radius >= p2.x && x - radius <= p2.x + p2.width) ? true : false;
        bool collisionRight = CheckCollisionCircleRec({x, y}, radius, {p1.x, p1.y, p1.width, p1.height});
        bool collisionLeft = CheckCollisionCircleRec({x, y}, radius, {p2.x, p2.y, p2.width, p2.height});

        if (overlapRight && y + radius >= p1.y && y - radius <= p1.y + p1.height)
        {
            while (true)
            {
        ...
        else if (overlapRight && collisionRight && y >= p1.y + p1.height)
        {
        ...
        if (overlapLeft && y + radius >= p2.y && y - radius <= p2.y + p2.height)
        {
            if (IsKeyDown(KEY_D))
            {
                x = p2.x + p2.width * 2 - 5;
...
int main()
{
    ...
    InitWindow(screen_width, screen_height, "Zbadj");
    SetTargetFPS(60);
    ...
    player1.width = 25;
    player1.height = 150;
    player1.size = {player1.width, player1.height};
    ...
    while (!WindowShouldClose())
    {
        ...
        player1.Update(KEY_UP, KEY_DOWN, KEY_LEFT, KEY_RIGHT, screen_width / 2 + 10, screen_width - (player1.width + 10));
        player2.Update(KEY_W, KEY_S, KEY_A, KEY_D, 10, screen_width / 2 - (player2.width + 10));
